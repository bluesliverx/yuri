...
class MessageDispatcher(object):
    def __init__(self, slackclient, plugins, errors_to):
        ...
        self.AT_MESSAGE_MATCHER = re.compile(
            r'^(?:\<@(?P<atuser>\w+)\>:?|(?P<username>\w+):{}) ?(?P<text>[\s\S]*)$'.format(alias_regex)
        )
    ...
    def dispatch_msg(self, msg):
        ...
        if not self._dispatch_msg_handler(category, msg, override_match_text):
            ...
            if (category == 'respond_to' and not msg.get('nlp_label')) or \
                ...
                if not self._dispatch_msg_handler('default_reply', msg, override_match_text):
                    self._default_reply(msg)

    def _dispatch_msg_handler(self, category, msg, override_match_text):
        responded = False
